...
bool Replica::checkAliveBlocking() {
    ...
    auto hbResp = connection.receiveResponse();
    if (!hbResp.has_value()) {
    ...
    if (hbResp1.type() == AdminMsgType::HEARTBEAT) {
        pthread_mutex_lock(&mutex);
        if (!isAlive) {
            printf("Replica %d is alive again!! It will be ignored "
                   "because it has an old state\n",
                   id);
        } else {
            isAlive = true;
            // Save the last heartbeat time
            lastHeartbeat = std::chrono::duration_cast<
        ...
        // What!?
        printf("Replica %d sent an invalid response to the "
               "heartbeat\n",
               id);
