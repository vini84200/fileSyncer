...
bool existe_sync_dir(const std::string &caminho) {
    return std::filesystem::exists(caminho) &&
           std::filesystem::is_directory(caminho);
}

void adiciona_watcher(std::string syncDirPath, int *file_descriptor,
                      int *wd) {
    ...
    *wd = inotify_add_watch(*file_descriptor, syncDirPath.c_str(),
                            IN_MODIFY | IN_CLOSE_WRITE | IN_DELETE);

    if (*wd < 0)
        errno == ENOENT ? std::cerr << "Diretório " << syncDirPath
                                    << " não encontrado" << std::endl
                        : std::cerr << "Erro ao adicionar o watch "
                                       "para o diretório "
                                    << syncDirPath << std::endl;
}

void verifica_modificacao(int *file_descriptor,
                          char buffer_inotify[BUF_INOTIFY_LEN]) {
    int length =
            read(*file_descriptor, buffer_inotify, BUF_INOTIFY_LEN);
    if (length < 0)
        std::cerr << "Erro ao ler eventos do inotify" << std::endl;

    for (int i = 0; i < length;) {
        struct inotify_event *event =
                (struct inotify_event *) &buffer_inotify[i];
        if (event->len) {
            if ((event->mask & 0) || (event->mask & IN_CLOSE_WRITE)) {
                ...
                std::cout << "Arquivo modificado: " << event->name
                          << std::endl;
                ...
                std::string newHash = digest_to_string(
                        getFileDigest(syncDirPath + "/" + filename));
                ...
                if (response.type() == ERROR) {
                    std::cerr << "Erro ao enviar arquivo para o "
                                 "servidor"
                              << std::endl;
                    return;
                }
                ...
                if (response.type() == FILE_UPDATED) {
                    std::cout << "Arquivo já atualizado no servidor"
                              << std::endl;
                    return;
                }
                ...
                if (response.type() == SEND_FILE_DATA) {
                    ...
                    auto maybeResponse = conn.receiveResponse();
                    if (!maybeResponse.has_value()) {
                        std::cerr << "Erro ao receber resposta do "
                                     "servidor"
                                  << std::endl;
                        return;
                    }
                    ...
                    if (response.type() == FILE_UPDATED) {
                        std::cout << "Arquivo enviado ao servidor "
                                     "com sucesso"
                                  << std::endl;
                    }
                }
                ...
                std::cout << "Outro evento: " << event->name
                          << std::endl;
            }
        }
...
int terminal_Interaction(char *command, char *file_path) {
    ...
    if (strcmp(command, "ping\0") == 0) { return ping(); }
...
int deleteFile(char *path) {
    ...
    auto [header, response] = maybeResponse.value();
    if (response.type() == ERROR) {
        std::cerr << "Erro ao enviar arquivo para o servidor"
                  << std::endl;
        return CONTINUE;
    }
    if (response.type() == FILE_UPDATED) {
...
int upload(char *path) {
    ...
    request.mutable_file_data_update()->set_data(
            std::string(std::istreambuf_iterator<char>(file),
                        std::istreambuf_iterator<char>()));
    request.mutable_file_data_update()
            ->mutable_file_update()
            ->set_filename(filename);
    request.mutable_file_data_update()
            ->mutable_file_update()
    ...
    request.mutable_file_data_update()
            ->mutable_file_update()
            ->set_hash(digest_to_string(hash));
    ...
    if (response.type() == FILE_UPDATED) {
        std::cout << "Arquivo enviado ao servidor com sucesso"
                  << std::endl;
    }
    return CONTINUE;
}
...
int ping() {
    ...
    auto [header, response] = maybeResponse.value();
    if (response.type() == ResponseType::ERROR) {
        printf("\nErro ao listar arquivos no servidor: ",
               response.error_msg().c_str());
        return CONTINUE;
    }
...
int listClient() {
    ...
    for (const auto &entry:
         std::filesystem::directory_iterator(syncDirPath))
...
void *thread_front_end(void *) {
    ...
    while (notExit) {
        printf("Digite o comando desejado ou 'help' para ver a lista "
               "de comandos\n > ");
...
void changeIP(std::string hostname, int port) {
    ...
    ConnectionArgs cArgs   = ConnectionArgs(hostname, port, username,
                                            password, sessionIdTemp);
    mainConn               = new ClientConnection(cArgs);
    ClientConnection &conn = *mainConn;


    pthread_t thread;
    pthread_create(&thread, NULL, thread_updates, NULL);
...
int main(int argc, char *argv[]) {
    ...
    int port;
    std::istringstream port_s(argv[3]);
    port_s >> port;

    // COMUNICATION

    ConnectionArgs cArgs =
            ConnectionArgs(argv[2], port, username, password);
    mainConn               = new ClientConnection(cArgs);
    ClientConnection &conn = *mainConn;

    {
        ...
        printf("Hostname: %s\n", hostname.c_str());
        request.mutable_frontend_subscription()->set_hostname(
                hostname);
        request.mutable_frontend_subscription()->set_port(
                DAEMON_PORT);
        ClientConnection conn(*mainConn);
        conn.sendRequest(request);
    ...
    pthread_t thread_monitoramento_id, thread_updates_id,
            thread_front_end_id, thread_daemon_listener_id;
    pthread_create(&thread_monitoramento_id, NULL,
                   thread_monitoramento, NULL);
    pthread_create(&thread_updates_id, NULL, thread_updates, NULL);
    pthread_create(&thread_front_end_id, NULL, thread_front_end,
                   NULL);
    pthread_create(&thread_daemon_listener_id, NULL,
                   thread_daemon_listener, NULL);
